...
def robust_ring3(cat, ref, area, radius, sigma, minpairs=None, gamma=None, mid=True, **kw):
    ...
    :type gamma:          float
    :type mid:            bool
    :returns:             (omega, pairs, w) omega: 3D transformation vector estimated in all pairs by the robust algorithm
                          pairs: all pairs matched within search radius
                          w: robust weights for all pairs.
    """
    
    # get all pairs within search radius
    pairs = getpairs(cat=cat, ref=ref, sep=radius/arc2rad)
    if pairs.shape[0] < 1:
        raise ValueError('robust_ring(): no pairs found')
    ringpairs = getRing2(cat=cat, ref=ref, pairs=pairs)
    bestomega, bestpairs, bestwt = process_ring2(cat, ref, pairs, ringpairs, area, radius, sigma, **kw)
    
    ## final estimation over all pairs
    r,c = getRC(cat=cat, ref=ref, pairs=pairs, mid=mid)
        
    # compute gamma for all pairs
...
def robust_ring2(cat, ref, area, radius, sigma, minpairs=None, **kw):
    """
    ...
    if pairs.shape[0] < 1:
        raise ValueError('robust_ring(): no pairs found')
    ringpairs = getRing2(cat, ref, pairs)
    return process_ring(cat, ref, pairs, ringpairs, area, radius, sigma, **kw)

def robust_ring1(cat, ref, area, radius, sigma, ringwidth=1, **kw):
    """
    Estimate omega with robust algorithm in rings. Obtain optimal estimate from best ring.
    This splits the pairs up using fixed-width rings.
    Additional keywords are passed to process_ring().
    ...
    :param radius:        Separation radius threshold.
    :param sigma:         True value of sigma, the astrometric uncertainty of the catalog.
    :param ringwidth:     Specified ring-width. Default is 1 arcsec.
    :type cat:            pandas.DataFrame
    :type ref:            pandas.DataFrame
    :type area:           float
    :type radius:         float
    ...
    # generate pairs in rings, within ringwidth
    ringpairs = getRing1(cat, ref, radius, pairs, ringwidth)
    return process_ring(cat=cat, ref=ref, pairs=pairs, ringpairs=ringpairs, area=area, radius=radius, sigma=sigma, **kw)

def getRing2(cat, ref, pairs, minpairs=None):
    """
...
def getRing1(cat, ref, radius, pairs, ringwidth=1):
    """
    Divide all pairs into rings given by the ring-width specified and return a list of pairs in the rings. 
    Rings overlap by 1/2 ringwidth, so they go from 0-ringwidth, 
    0.5*ringwidth-1.5*ringwidth, ringwidth-2*ringwidth, etc.
    
    :param cat:       Input catalog dataframe with (x,y,z) coordinates.
    :param ref:       Reference catalog dataframe with (x,y,z) coordinates.
    :param radius:    Separation radius threshold. If the distance between a pair of sources 
                      less than radius, it is considered as a match.
    :param pairs:     Match index of pairs within radius.
    :param ringwidth: Specified ring-width. Default is 1 arcsec. 
    :type cat:        pandas.DataFrame
    :type ref:        pandas.DataFrame
    ...
    hwidth = 0.5*ringwidth
    nrings = int(radius/hwidth)-1
    if nrings<1:
        nrings = 1
    rinner = np.arange(nrings)*hwidth # in arcsec
    router = rinner+ringwidth
    if False: # substitute ?????
        rbin = np.linspace(0, radius, nrings) 
        rinner, router = rbin[:-1], rbin[1:]
    ...
    # interpolate in sep to find segments for each ring
    klo = sep.searchsorted(rinner,'left').clip(0, len(sep))
    khi = sep.searchsorted(router,'right').clip(klo, len(sep))
    rpairs = [pairs[klo[iring]:khi[iring],:] for iring in range(nrings)]
    
    return rpairs

def process_ring2(cat, ref, pairs, ringpairs, area, radius, sigma, sigma_init=None, gamma=None, niter=10, nextr=100, mid=True,
    ...
    :param gamma:         Fraction of good matches among all pairs in the ring. If None, will be computed in estimation.
    :param niter:         Min number of iterations for the convergence.
    :param nextr:         Max number of additional iterations for the convergence.
    ...
    for iring in range(nrings):
        ...
        # number of sources in catalog and reference
        numcat = len(set(rpairs[:,0]))
        numref = len(set(rpairs[:,1]))
        
        # compute gamma for each ring
        gamma = gamma or min(numcat, numref) / rpairs.shape[0]
